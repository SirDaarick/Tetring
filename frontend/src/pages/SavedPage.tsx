...
import { Star, Trash2, Calendar, LayoutGrid, Table as TableIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { ScheduleVisualGrid } from "@/components/scheduler/ScheduleCard";
...
function SavedPage(): ReactElement {
  ...
  function getViewMode(id: string): "grid" | "table" {
    return viewModes[id] || "grid";
  }

  function setViewMode(id: string, mode: "grid" | "table"): void {
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          {schedules.map((schedule, idx) => {
            const currentMode = getViewMode(schedule.id);
            const mappedGroups = schedule.groups.map((g) => {
              const days = [];
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4 pb-3 border-b border-clay-border/10">
                  ...
                  <div className="flex items-center gap-3">
                    {/* Conmutador de vista Bloques / Tabla */}
                    <div className="flex items-center rounded-clay border border-clay-border bg-white/50 p-0.5 shadow-clay-input">
                      <button
                        type="button"
                        onClick={() => setViewMode(schedule.id, "grid")}
                      ...
                      <button
                        type="button"
                        onClick={() => setViewMode(schedule.id, "table")}
                        className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold transition-all ${
                          currentMode === "table"
                            ? "bg-white text-clay-primary shadow-clay"
                            : "text-clay-text-secondary hover:text-clay-text"
                ...
                {currentMode === "grid" ? (
                  <div className="pt-2">
                    <ScheduleVisualGrid groups={mappedGroups} />
                  </div>
                ) : (
